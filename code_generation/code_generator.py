...
class Frame(object):
    ...
    def __init__(self, params, is_static, ret_type):
        """Initialise the frame once visiting a new method node. Static methods
        operate differently, because the first element on the static is not a
        reference to "this".
        """
    ...
    def get_var(self, var):
        """For a given variable name, gets the location in the array of local
        variables.
        """
...
class CodeGenerator(object):
    ...
    def __init__(self):
        ...
        self._cur_class = ''
        # Stores method signatures for simple retrieval when the method is
        # called.  Elements are indexed with a (class, method) tuple
        self._method_sigs = dict()
        # Stores field sigs
        self._field_sigs = dict()

    def compile_(self, source, dst = None):
        """This is the public method which takes the source file or string,
        and generates the assembly code file.
        """
        ...
        for ast in asts:
            ...
            out_file.close()
            # Run Jasmin
            jasmin_file = os.path.join(project_root, 'jasmin', 'jasmin.jar')
    ...
    def _gen_field_method_sigs(self, t_env):
        ...
        analyser.
        """
        for class_s in t_env.classes.values() + t_env.interfaces.values():
    ...
    def _gen_cons_sig(self, class_s, t_env):
        ...
        try:
            ...
            if cons is not None and len(cons.params) > 0:
                signature += '<init>'
                signature += self._gen_method_descriptor(cons, True)
            else:
    ...
    def _visit_class_node(self, node):
        ...
        try:
            for interface in children[2].children:
                self._add_ln('.implements ' + interface.value)
        except AttributeError:
    ...
    def _gen_method_def(self, node):
        ...
        """
        children = node.children
        # These nodes are stored in different locations depending on if the
        # method returns an array or not, that is why they are stored in these
        # auxiliary variables
    ...
    def _visit_super_constructor_call_node(self, node):
        ...
        try:
            ...
            self._gen_args_list_node(args_list, super_params)
        except AttributeError:
            # No argumnts
            pass
    ...
    def _gen_array_element_assignment(self, node):
        ...
        if isinstance(children[0], nodes.ArrayDclNode):
            # If it's an array declaration node, it must be treated like a
            # normal variable
    ...
    def _add_convert_op(self, l_child, r_child):
        """Helper method to convert one type to another before assignment if
        needed.  Works for type-tagged tree nodes, or string type descriptions.
        """
        l_type = l_child
        r_type = r_child
        ...
        if l_type != r_type and is_prim:
            ...
            if convert_op is not None:
                self._add_iln(convert_op, ';Convert left hand side to ' +
    ...
    def _gen_eq_to(self, node, type_, next_comp):
        if type_ in ['byte', 'char', 'short', 'int']:
            self._add_iln('if_icmpeq CompTrue' + next_comp,
                          ';If the value on the top of the stack is equal ' +
    ...
    def _gen_not_eq_to(self, node, type_, next_comp):
        if type_ in ['byte', 'char', 'short', 'int']:
            self._add_iln('if_icmpne CompTrue' + next_comp,
                          ';If the value on the top of the stack is is not ' +
    ...
    def _gen_comp_object(self, node):
        ...
        if op == '!=':
            self._add_iln('if_acmpne CompTrue' + next_comp,
                          ...
                          'the same instance as the one below it, jump to ' +
                          'CompTrue')
        # Generate the remainder of the comparison code
        self._gen_comp_end(next_comp)
    ...
    def _gen_comp_end(self, next_comp):
        ...
        """
        self._add_iln('ldc 0',
                      ';Comparison was false, so load binary constant 0')
    ...
    def _gen_matrix_operation(self, node):
        ...
        self._add_iln('dup', ';Dup to specify dimensions for the result ' +
                      'array as well as storing')
        ...
        if node.value == '*':
            self._gen_check_matrix_mult_dimensions(col_len_loc1, row_len_loc2)
        else:
            self._gen_check_matrix_add_dimensions(row_len_loc1, col_len_loc1,
        ...
        self._add_iln('if_icmpge MatRowEnd' + next_mat_rows,
                      ';Check if idx has reached the size of the rows')
        if node.value == '*':
            self._gen_matrix_multiplication(node, col_len_loc1, col_len_loc2,
                                            result_mat_loc, idx1_loc,
                                            next_mat_cols_b)
        else:
            self._gen_matrix_addition(node, col_len_loc2, result_mat_loc,
                                      idx1_loc, next_mat_cols_b)
        self._add_iln('iinc ' + idx1_loc + ' 1', ';Increment the index')
    ...
    def _gen_check_matrix_mult_dimensions(self, col_len_loc1, row_len_loc2):
        ...
        """
        next_comp = self._label_suffix('comp')
        self._add_iln('iload ' + col_len_loc1, ";Load first array's columns")
        self._add_iln('iload ' + row_len_loc2, ";Load second array's rows")
        self._add_iln('if_icmpeq CompTrue' + next_comp,
                      ';Check dimensions match')
        self._add_iln('new java/lang/ArithmeticException',
                      ';Create a new exception')
        self._add_iln('dup', ';Dup in order to call constructor, and throw')
    ...
    def _gen_check_matrix_add_dimensions(self, row_len_loc1, col_len_loc1,
        ...
        # Compare rows
        self._add_iln('iload ' + row_len_loc1, ";Load first array's rows")
        self._add_iln('iload ' + row_len_loc2, ";Load second array's rows")
        self._add_iln('if_icmpne CompTrue' + next_comp1,
                      ";If dimensions don't match, jump to " +
                      'exception throwing section')
        # Compare columns
        self._add_iln('iload ' + col_len_loc1, ";Load first array's cols")
        self._add_iln('iload ' + col_len_loc2, ";Load second array's cols")
        self._add_iln('if_icmpeq CompTrue' + next_comp2,
                      ';Check dimensions match')
        self._add_ln('CompTrue' + next_comp1 + ':', ';First check failed')
        self._add_iln('new java/lang/ArithmeticException',
                      ';Create a new exception')
        self._add_iln('dup', ';Dup in order to call constructor, and throw')
    ...
    def _gen_matrix_addition(self, node, col_len_loc, result_mat_loc,
        ...
        self._add_iln('if_icmpge MatColEnd' + next_mat_cols,
                      ';Check if idx has reached the size of the cols')
        # Do the addition
        # Load the result matrix
        self._add_iln('aload ' + result_mat_loc, ';Load the result matrix')
        ...
        # End the loops
        self._add_iln('iinc ' + idx2_loc + ' 1', ';Increment the index')
    ...
    def _gen_matrix_multiplication(self, node, col_len_loc1, col_len_loc2,
        ...
        self._add_iln('iinc ' + idx3_loc + ' 1', ';Increment the index')
    ...
    def _gen_arith(self, node):
        ...
        # Simply apply appropriate asm instruction to the values on the top of
        # the stack
        op = ''
        if node.value == '+':
    ...
    def _visit_not_node(self, node):
        ...
        self._add_ln('IsFalse' + next_not + ':', ';Start here to change to 1')
    ...
    def _visit_inc_node(self, node):
        ...
        try:
            ...
            # First get the element of the array to store into on the stack
            var_name = children[0].children[0].value
            ...
            # Store the new value back in the cell originally pushed on the
            # stack
    ...
    def _visit_method_call_node(self, node):
        ...
        try:
            ...
            method_s = self._find_lib_method_s(self._cur_class, method_name,
                                               node.children[1])
    ...
    def _visit_method_call_long_node(self, node):
        ...
        try:
            ...
            method_s = self._find_lib_method_s(class_name, method_name,
                                               args_list)
        # Invoke the method
        ...
        if class_s is not None and class_s.name in self._t_env.classes:
            ...
            try:
                ...
                # first is the ref to the current object
                num_args = 1
                try:
                ...
                self._add_iln('invokeinterface ' +
                              self._method_sigs[(class_s.name, method_s.name)]
                              + ' ' + str(num_args))
    ...
    def _visit_method_call_super_node(self, node):
        ...
        # Get the results from what the arguments are on the top of the stack
        try:
            self._gen_args_list_node(node.children[-1], method_s.params)
        except AttributeError:
            # No args
            pass
        # The operator needs to be invoke special if there is a method with the
        # same name in the current class
        op = 'invokevirtual'
    ...
    def _find_lib_method_s(self, class_name, method_name, args_list):
        ...
        """
        # If the class provided is not the name of a library class, recursively
        # look through its super classes until a library class is reached
        try:
            super_name = ''
            ...
            try:
                ...
                # It was actually an interface
                super_name = class_s.super_interface
            return self._find_lib_method_s(super_name, method_name, args_list)
        except SymbolNotFoundError:
    ...
    def _gen_args_list_node(self, args, params):
        """Generate code for the arguments.  It takes the parameters of the
        method that the arguments are being passed into, to check if any type
    ...
    def _visit_object_creator_node(self, node):
        ...
        try:
            ...
            try:
                ...
                self._gen_args_list_node(args_list, cons_s.params)
            except IndexError:
                # No arguments
                pass
    ...
    def _gen_load_variable(self, node):
        ...
        try:
            ...
            self._add_iln(self._prefix(node) + 'load ' + var, ';Load value ' +
                          'stored in ' + var + ' (' + node.value + ')')
        except KeyError:
            # It must be a field
            self._add_iln('aload_0',
                          ';Load "this" in order to access the field')
    ...
    def _visit_literal_node(self, node):
        ...
        if node.type_ == 'byte':
            self._add_iln('bipush ' +  str(node.value),
                          ';Load constant numerical byte ' + str(node.value))
        elif node.type_ == 'short':
            self._add_iln('sipush ' +  str(node.value),
                          ';Load constant numerical short ' + str(node.value))
        if node.type_ in ['char', 'int', 'float']:
            self._add_iln('ldc ' +  str(node.value),
                          ';Load constant numerical value ' + str(node.value))
        elif node.type_ in ['long', 'double']:
            self._add_iln('ldc2_w ' +  str(node.value),
                          ';Load constant numerical value ' + str(node.value))
        elif node.type_ == 'java/lang/String':
            self._add_iln('ldc "' + node.value + '"', ';Load constant string "'
                          + node.value + '" (creates a new String object)')
