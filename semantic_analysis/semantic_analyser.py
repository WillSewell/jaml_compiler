...
class TypeChecker(object):
    ...
    def __init__(self):
        ...
        # Provides a more convenient way of calling these commonly used methods
    ...
    def _visit_class_body_node(self, node, env):
        ...
        the child nodes and type check the children.
        """
        prev_table = env
        ...
        # If the super class has a constructor with arguments, this class must
        # have a constructor that calls the super classes constructor
        # At this stage we just check if the current class has a constructor
        # at all, in the code or constructor nodes, we check the call to the
        # super constructor is actually made
        ...
        try:
            ...
            if super_cons != None and len(super_cons.params) != 0:
                ...
                if not isinstance(super_node,
                    ...
                    msg = ('The super classes constructor must be invoked ' +
                           'on the first line!')
                    raise ConstructorError(msg)
                # Check the types of the arguments match
                self._check_method_args(super_cons, super_node.children[0], env)
        except SymbolNotFoundError:
            # No explicit super class
            pass
        # Type check the body
    ...
    def _check_override(self, name, env):
        ...
        try:
            ...
            if 'final' in super_method.modifiers:
                msg = 'Method "' + name + '" cannot override a final method!'
                raise MethodSignatureError(msg)
        except (SymbolNotFoundError, AttributeError):
            # It does not override a method, or there was no superclass
            # (that wasn't a library class)
            pass

    def _visit_param_list_node(self, node, env):
        """The types of the parameters must be checked
    ...
    def _check_method_return(self, node, env):
        ...
        if node.children[1].value != 'void':
            if not env.cur_method.has_ret:
                msg = 'Method "' + node.children[0].value + '" has no return!'
    ...
    def _gen_var_s(self, node, env):
        ...
        # Check the variable does not have the same name as a type, field or
        # parameter
    ...
    def _visit_assign_node(self, node, env): # TODO: CAN'T ASSIGN TO A FIELD IN ANOTHER CLASS (CAN'T INC IT TOO)
        ...
        if name != '':
            ...
            if symbol.is_init == False:
                msg = 'Array "' + symbol.name + '" not initialised!'
    ...
    def _check_final(self, symbol):
        ...
        try:
            if 'final' in symbol.modifiers:
                msg = 'Cannot assign to final field "' + symbol.name + '"!'
    ...
    def _check_is_assignable(self, type1, type2, env):
        ...
        try:
            ...
            try:
                ...
                if dimensions1 != dimensions2:
                    msg = 'Number of array dimensions incorrect!'
                    raise DimensionsError(msg)
                # Set the types to the array's type
                type1 = type1.type_
                type2 = type2.type_
            except AttributeError:
                msg = 'Type being assigned or returned must be an array!'
                raise TypeError(msg)
        except AttributeError:
            # Check the other side isn't an array type
            if isinstance(type2, ArrayType):
                msg = 'Type being assigned or returned cannot be an array!'
                raise TypeError(msg)
        ...
        if type1 in self._t_env.nums:
            ...
            # instantiation
                msg = ('Type "' + type2 +
                       '" incompatible for assignment or returning.')
                raise TypeError(msg)
    ...
    def _check_num_types(self, type1, type2):
        ...
        """
        try:
            # Numbers that can hold a greater value can have a type which holds
            # less assigned to them.  Types with a lower index value in the
            # list of numbers can be assigned to ones with a higher index.
    ...
    def _is_equal_types(self, lh_type, rh_type):
        """Helper method for _check_is_assignable.  Checks if the right hand
        ...
        if lh_type == rh_type:
            ...
            if len(rh_class.interfaces) != 0:
                for interface in rh_class.interfaces:
                    return self._is_equal_types(lh_type, interface)
            else:
    ...
    def _visit_eq_node(self, node, env):
        ...
        if l_child_type in self._t_env.nums:
            if r_child_type not in self._t_env.nums:
                msg = ('Type error in equality node, ' +
                       'second child was not a number!')
                raise TypeError(msg)
    ...
    def _visit_rel_node(self, node, env):
        """
        Check the type of the node with regex because there is a number of
        possibilities.
        """
    ...
    def _visit_add_node(self, node, env):
        ...
        if lh_type == 'matrix':
            if rh_type != 'matrix':
                msg = ('Type error in additive node, ' +
                       'right child must be a matrix!')
                raise TypeError(msg)
            else:
                node.type_ = 'matrix'
            ...
            if rh_type not in self._t_env.nums:
                msg = ('Type error in additive node, ' +
                       'right child is not a number!')
                raise TypeError(msg)
            else:
                get_type = self._num_operator_type
                node.type_ =  get_type(node.children[0], node.children[1])
        # If it's used to concatenate strings, check they match
            ...
            msg = 'Type error in + node, type must be String or a number!'
            raise TypeError(msg)
        return node.type_
    ...
    def _visit_mul_node(self, node, env):
        ...
        if node.value == '*' and lh_type == 'matrix':
            if rh_type != 'matrix':
                msg = 'Type error at *, right child must be a matrix!'
                raise TypeError(msg)
            else:
                node.type_ = 'matrix'
    ...
    def _num_operator_type(self, l_child, r_child):
        ...
        """
        # Work out each numbers index in the list of numbers, lower index
        ...
        if l_child.type_ == r_child.type_:
            ...
            # Otherwise, return the type of whatever is higher up in the list
            # of possible numerical types
            return l_child.type_
        else:
    ...
    def _visit_inc_node(self, node, env):
        ...
        try:
            ...
            if 'static' in env.cur_method.modifiers:
                is_static = True
            symbol = self._find_field(env.cur_class.name, field_name,
                                      is_static, env)
            self._check_final(symbol)
        except SymbolNotFoundError:
            # It was not a field
    ...
    def _visit_array_dcl_node(self, node, env):
        ...
        symbol = self._get_var_s_from_id(node.children[0].value, env, False)
    ...
    def _visit_array_element_node(self, node, env):
        """Check the array has been declared, and the number of dimensions
        match.
        """
        # First check it has been declared by looking it up in env
        # (also getting its type)
        symbol = self._get_var_s_from_id(node.children[0].value, env, True)
        # Check the dimensions are equal to the dimensions stored in env
        if symbol.dimensions != len(node.children) - 1:
            msg = ('Array "' + node.children[0].value + '" does not have ' +
                   str(len(node.children) - 1) + ' dimensions!')
            raise TypeError(msg)
        # Return the type part of the list use to represent the type of the
        # whole array because this is a reference to an instance of an array
    ...
    def _visit_method_call_node(self, node, env):
        """Check the method exists, the types are the same, and the argument's
        match the method's signature.
        """
        # Look up the method in this class, or a super class
        node.type_ = self._find_method(env.cur_class.name, node, False, env)
        return node.type_

    def _visit_method_call_long_node(self, node, env):
        """Works in much the same way has the above method, but this must look
        up methods in other classes.
        """
    ...
    def _find_method(self, class_, node, is_static, env):
        ...
        try:
            ...
            try:
                ...
                # are second child
                self._check_method_args(method_s, node.children[1], env)
    ...
    def _check_static(self, symbol, is_static):
        ...
        if is_static and 'static' not in symbol.modifiers:
            msg = 'Cannot access non static method/field in a static way!'
    ...
    def _check_method_args(self, method_s, args_list, env):
        """Helper method for _visit_method_call.  Checks the number and types
        of the arguments match.
        """
        try:
            # If the method call has arguments
            self._check_method_args_types(method_s, args_list, env)
        except AttributeError:
    ...
    def _check_lib_method(self, class_, node, env):
        ...
        # Create a symbol and add it to the t_env for the code generator to use
        # later
        symbol = LibMethodSymbol(method_name, ret_type, arg_types, class_,
                                 parent_class, is_static)
        if is_static:
    ...
    def _get_arg_types(self, node, env):
        ...
        try:
            ...
            for arg in args:
                type_ = arg.type_
                try:
                    # Array types need to be converted to JVM type representation
                    type_.type_
                    get_jvm_type(type_)
    ...
    def _visit_field_ref_node(self, node, env):
        ...
        if isinstance(class_name, ArrayType):
            if field_name in 'length':
                msg = 'Arrays have no fields other than length!'
                raise SymbolNotFoundError(msg)
            else:
                node.type_ = 'int'
        elif class_name == 'matrix':
            if field_name not in ['rowLength', 'colLength']:
                msg = ('Matrices have no fields other than rowLength and ' +
                       'colLength!')
                raise SymbolNotFoundError(msg)
            else:
                node.type_ = 'int'
        ...
        try:
            if env.cur_class != None:
                ...
                if 'final' in super_s.modifiers:
                    msg = 'You cannot subclass a final class!'
    ...
    def _get_var_s_from_id(self, name, env, check_init):
        ...
        try:
            ...
            if check_init and env.get_var_s(name).is_init == False:
                msg = 'Variable "' + name + '" used without being initialised!'
                raise NotInitWarning(msg)
            ...
            try:
                ...
                symbol = self._find_field(cname, name, is_static, env)
            except SymbolNotFoundError:
    ...
    def _check_lib_field(self, class_, field, env):
        ...
        dotted_name = full_class_name.replace('/', '.')
        check = self._run_lib_checker
        type_, containing_class, is_static = check(['-field', dotted_name,
                                                    field])
        if type_[0] == '[':
            # It's an array, so must be converted to the array class
            convert = self._convert_jvm_array_type_to_class
    ...
    def _visit_literal_node(self, node, env):
        ...
        # Convert the node class name into a string description of the type of
        # the node
    ...
    def _visit_abs_method_dcl_node(self, node, env):
        ...
        if env.cur_class != None:
            ...
            if 'abstract' not in class_s.modifiers:
                msg = ('Class "' + class_s.name + '" must declared abstract ' +
    ...
    def _run_lib_checker(self, args):
        ...
        if output[0] == 'E':
            ...
            if args[0] in ['-method', '-field']:
                # The checker prints both the type, and the containing class
                # for methods and fields - these need to be extracted here
                split = output.split('|')
                # Convert string representation of whether or not it's static
                # to Python boolean type
                if split[2] == 'true':
