"""The test class for the the semantic analysis module."""
import unittest
import os
...
class TestSemanticAnalyser(unittest.TestCase):
    ...
    def test_scan_classes_methods(self):
        """Test that all referenced classes are scanned properly, and returned.
        """
    ...
    def test_class_extends_final_fail(self):
        """Test error thrown when a final class is attempted to be extended.
        """
        self.assertRaises(ClassSignatureError, self.analyse_file,
                          'test_class_extends_final_fail.jml')

    def test_implements_pass(self):
        """Test that no errors thrown when a class implements more than one
        interface correctly.
        """
    ...
    def test_not_implemented_fail(self):
        """Test the case where a class does not correctly implement all
    ...
    def test_env_pass(self):
        """Test that a variable is correctly looked up when it's used in a
        nested block.
        """
        self.analyse_stmt('int x = 4;if (x == 5){ x = 3;}')

    def test_env_fail(self):
        """Test that an exception is raised when a variable is referenced that
        was declared in an inner block.
        """
        self.assertRaises(SymbolNotFoundError, self.analyse_stmt,
                          'if ("y" != "x"){float x = 10L;}x = 15L;')

    def test_class_scope_fail(self):
        """Test exception thrown when variable is referenced from another
        method.
        """
        self.assertRaises(SymbolNotFoundError, self._analyse,
    ...
    def test_param_pass(self):
        ...
        self._analyse('class X { void x(int y) {int x = y;}}')

    def test_param_fail(self):
        """Test parameters of one method are not accessible elsewhere."""
        self.assertRaises(SymbolNotFoundError, self._analyse,
                          'class x { void x(int y) {} void z(){int w = y;}}')
    ...
    def test_super_cons_params_no_params(self):
        """Test that an error is thrown when the super class has a constructor
        which takes parameters, and the sub class has a constructor, but it
        does not call the super classes constructor.
        """
        self.assertRaises(ConstructorError, self.analyse_file,
                          'test_super_cons_params_no_params.jml')

    def test_multi_constructors_fail(self):
        """Test an error is thrown when a class contains multiple constructors.
        """
        self.assertRaises(ConstructorError, self._analyse,
                          'class X { X() {} X() {} }')

    def test_method_call_pass(self):
        """Test that there is no error when a call is made to another method
        within the same class.
        """
        self._analyse('class X {void x() {y();} void y(){} }')

    def test_method_call_fail(self):
        """Test an error is thrown when a method is called which doesn't exist.
        """
        self.assertRaises(SymbolNotFoundError, self._analyse,
                          'class X {void x() {z();} void y(){} }')

    def test_method_call_args_pass(self):
        """Test that there is no error when the arguments match the method
        signature.
        """
        self._analyse('class X {void x() {long w = 5; y(w + 5);} ' +
                      'void y(long z){} }')

    def test_method_call_args_length_fail(self):
        """Test an error is thrown when a method is called where no arguments
        are provided when there should be one.
        """
        self.assertRaises(MethodSignatureError, self._analyse,
                          'class X {void x() {y();} void y(long z){} }')

    def test_method_call_args_wrong_type_fail(self):
        """Test an error is thrown when a method is called where the arguments
    ...
    def test_method_return_pass(self):
        """Test that no error is raised when the correct type is returned."""
        self._analyse('class X {short x(){return 6;}}')

    def test_method_no_return_fail(self):
        """Test that an error is raised when a method does not return anything
        when it's signature states it should.
        """
        self.assertRaises(NoReturnError, self._analyse,
                          'class X {short x(){}}')

    def test_method_wrong_return_fail(self):
        """Test that an error is raised when a method returns an incorrect
        type.
        """
        self.assertRaises(TypeError, self._analyse,
    ...
    def test_method_return_array_pass(self):
        ...
        self._analyse('class X {byte[][] x(){return new byte[5][5];}}')
    ...
    def test_method_call_external_pass(self):
        """Test no error thrown when a method is called from an object of
        another class."""
        self.analyse_file('test_method_call_external_pass.jml')

    def test_method_call_external_not_exists_fail(self):
        """Test that an error is raised when a method is called in  another
        class which doesn't exist."""
        self.assertRaises(SymbolNotFoundError, self.analyse_file,
                          'test_method_call_external_not_exists_fail.jml')

    def test_method_call_external_wrong_type_fail(self):
        """Test that an error is raised when a method is called in another
        class which returns an incompatible type with what the result is being
        assigned to."""
        self.assertRaises(TypeError, self.analyse_file,
                          'test_method_call_external_wrong_type_fail.jml')

    def test_method_call_super_pass(self):
        """Test no error thrown when a method is invoked that is defined in a
        super class.
        """
        self.analyse_file('test_method_call_super_pass.jml')

    def test_method_override_final_fail(self):
        """Test that an error is thrown when a subclass attempts to override a
    ...
    def test_method_call_static_pass(self):
        """Test no error thrown when a static method is called from another
        class.
        """
        self.analyse_file('test_method_call_static_pass.jml')

    def test_method_call_static_not_static_fail(self):
        """Test an error thrown when a method is referenced in a static way
        when it is not static.
        """
    ...
    def test_lib_method_call_fail(self):
        """Test a call to a library class fails when the method does not exist
        in that class."""
        self.assertRaises(SymbolNotFoundError, self._analyse,
    ...
    def test_lib_method_interface_call_fail(self):
        """Test a method invokation on an interface, where the object has the
        method, but the interface does not.
        """
        self.assertRaises(SymbolNotFoundError, self._analyse,
    ...
    def test_ref_non_static_field_fail(self):
        """Test that an error is thrown when a non static field is referenced
        from a static method.
        """
        self.assertRaises(StaticError, self._analyse,
                          'class X {int x; X(){x=5;} static void x()' +
                          '{ int y = x;}}')

    def test_pivate_method_pass(self):
        """Test no error thrown when a private method is called from within
        the same class.
        """
        self._analyse('class X { void x() {} void y() { x();} }')

    def test_private_method_fail(self):
        """Test an error thrown when a private method is attempted to be
        accessed in another class.
        """
        self.assertRaises(SymbolNotFoundError, self.analyse_file,
    ...
    def test_object_creator_abstact_fail(self):
        """Test an error is thrown when an abstract class is attempted to be
        instantiated.
        """
    ...
    def test_field(self):
        ...
        self._analyse('class X { int x; void x() {x = 5;}}')

    def test_final_field_assign_fail(self):
        """Test error thrown when assignment is attempted with a final field.
        """
    ...
    def test_field_ref_external_pass(self):
        """Test no error thrown when a field is referenced in another class.
        """
        self.analyse_file('test_field_ref_external_pass.jml')

    def test_field_ref_external_fail(self):
        """Test an error thrown when a field is referenced in another class
        that doesn't exist.
        """
        self.assertRaises(SymbolNotFoundError, self.analyse_file,
    ...
    def test_field_ref_static_not_static_fail(self):
        """Test an error thrown when a field is referenced in a static way
        when it is not static.
        """
        self.assertRaises(StaticError, self.analyse_file,
    ...
    def test_field_ref_super_pass(self):
        """Test no error thrown when a field is referenced that is defined in
        a super class.
        """
        self.analyse_file('test_field_ref_super_pass.jml')

    def test_pivate_field_pass(self):
        """Test no error thrown when a private field is used in the same class.
        """
    ...
    def test_lib_field_ref(self):
        ...
        """
        self._analyse('class X { void x() { int x = Thread.MAX_PRIORITY;}}')

    def test_lib_field_ref_fail(self):
        """Test that an error is thrown when a field doesn't exist in a
        library class.
        """
        self.assertRaises(SymbolNotFoundError, self._analyse,
                          'class X {void x(){InputStream x = ' +
    ...
    def test_name_clash_field_type(self):
        """Test an error thrown when there is a name clash between a type
        (class) name and a field name.
        """
        self.assertRaises(VariableNameError, self._analyse,
                          'class X { short X; }')

    def test_name_clash_param_field(self):
        """Test an error thrown when there is a name clash between a parameter
    ...
    def test_name_clash_local_param(self):
        """Test an error thrown when there is a name clash between a parameter
        name and a local variable name.
        """
        self.assertRaises(VariableNameError, self._analyse,
                          'class X { void y(byte x){int x = 5;} }')

    def test_name_clash_local_field(self):
        """Test an error thrown when there is a name clash between a field
        name and a local variable name."""
        self.assertRaises(VariableNameError, self._analyse,
                          'class X { double x; void y(){int x = 5;} }')

    def test_if_pass(self):
        """Test no exception thrown when there is a boolean expression in the
        if statement.
        """
        self.analyse_stmt('if(3 > 1){}')

    def test_if_fail(self):
        """Test exception thrown when there is no boolean expression in the if
        statement.
        """
    ...
    def test_while_fail(self):
        """Test exception thrown when there is no boolean expression in the
        while statement.
        """
        self.assertRaises(TypeError, self.analyse_stmt, 'while(3 - 1){}')
    ...
    def test_assign_pass(self):
        """Test no exception thrown when the types of both child nodes are
        equal.
        """
        self.analyse_stmt('int x = 1;')

    def test_assign_undeclared_fail(self):
        """Test exception thrown when an assignment is made, but the variable
    ...
    def test_array_init_pass(self):
        """Test no exception thrown when the type and dimension of the array
        is the same.
        """
        self.analyse_stmt('int arr[] = new int[10];')

    def test_array_init_type_fail(self):
        """Test exception thrown when the type of an array initialisation is
        incorrect.
        """
        self.assertRaises(TypeError, self.analyse_stmt,
    ...
    def test_array_init_dimension_fail(self):
        """Test exception thrown when the number of dimensions of an array
        initialisation is incorrect.
        """
    ...
    def test_array_element_assign_type_fail(self):
        """Test exception thrown when the type of an array element assignment
        is incorrent."""
        self.assertRaises(TypeError, self.analyse_stmt,
    ...
    def test_array_element_assign_dimension_fail(self):
        """Test exception thrown when the number of dimensions of an array
        element assignment is incorrent.
        """
        self.assertRaises(TypeError, self.analyse_stmt,
                          'int arr[] = new int[10]; arr[1][2] = 5;')

    def test_array_assign_undeclared_fail(self):
        """Test exception thrown when an array element assignment is made, but
    ...
    def test_matrix_element_assign_pass(self):
        """Test no exception thrown when a matrix element is assigned to."""
    ...
    def test_matrix_assign_undeclared_fail(self):
        """Test exception thrown when a matrix element assignment is made, but
    ...
    def test_cond_fail(self):
        """Test exception thrown when the child nodes are not boolean."""
    ...
    def test_relational_pass(self):
        """Test no exception thrown when the types of both child nodes are int.
        """
    ...
    def test_concat_fail(self):
        """Test that an error is thrown when a string is concatenated with a
        non string.
        """
    ...
    def test_not_pass(self):
        """Test no exception thrown when the type of the child node is boolean.
        """
    ...
    def test_neg_pass(self):
        """Test no exception thrown when the type of the child node is int."""
        self.analyse_stmt('-1;')
    ...
    def test_convert_int_long(self):
        """Test an int can be divided by a long and the / node's type becomes
        long.
        """
    ...
    def test_convert_long_float(self):
        """Test a long can be added to a float and the + node's type becomes
        float.
        """
    ...
    def test_convert_int_double(self):
        """Test a float can be subtracted by a double and the - node's type
        becomes double.
        """
    ...
    def analyse_stmt(self, stmt):
        """Helper method used so that single line code can be tested without
        having to declare a class and method for it to go in.
        """
